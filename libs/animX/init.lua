...
function animx.newAnimation(params)
	...
	setmetatable(animation_obj,{__index=Animation})

	-- Always add to global list for animx.update() to work
	table.insert(animx.animObjs, animation_obj)
...
function animx.newAnimationXML(image,filename)
	...
	for line in love.filesystem.lines(filename) do
		if i>1 and line:match('%a') and not line:match('<!') and line~="</TextureAtlas>" then
			...
			if not frameNo or frameNo<=0 then
				print("avoid to use goto in anumX")
				-- goto continue
			else
...
function animx.newActorXML(image,filename)
	...
	for line in love.filesystem.lines(filename) do
		if i>1 and line:match('%a') and not line:match('<!') and line~="</TextureAtlas>" then
			...
			if not animName or not frameNo or frameNo<=0 then
				-- goto continue
				print("avoid to use goto in anumX")
			else
...
function animx.update(dt)
	for i=1,#animx.animObjs do
		if animx.animObjs[i] then
			animx.animObjs[i]:update(dt)
		end
	end
end

--Manually clear the image cache (useful for freeing memory)
function animx.clearImageCache()
	for k in pairs(imgCache) do
		imgCache[k] = nil
	end
	collectgarbage("collect")
end

--Remove a specific animation from the global list
function animx.removeAnimation(anim)
	for i = #animx.animObjs, 1, -1 do
		if animx.animObjs[i] == anim then
			table.remove(animx.animObjs, i)
			return true
		end
	end
	return false
...
--Clear all animations (useful for scene changes)
function animx.clearAllAnimations()
	animx.animObjs = {}
end
